...
def getcampaigns(request,ipaddress):
    try:
        ...
        elif campaign.send_from == 'SMTP':
            accountsObjects = SMTPConfiguration.objects.filter(user=campaign.user)
            if not accountsObjects:
                return JsonResponse({'status':False,'data':{'error':'No SMTP Accounts Added'}}, safe=False)
            for acc in accountsObjects:
...
from .serializers import EmailAccountsSerializer
from django.utils.timezone import now
from datetime import timedelta

class EmailAccountsAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        for account_data in data:
            serializer = EmailAccountsSerializer(data=account_data)
            if serializer.is_valid():
                try:
                    user = User.objects.get(username=serializer.validated_data['user'])
                except User.DoesNotExist:
                    return Response(
                        {"error": f"User '{serializer.validated_data['user']}' does not exist."},
                        status=status.HTTP_400_BAD_REQUEST
                    )

                # Check if an EmailAccount exists for this user and email
                ...
                if not created:  # If it already exists, update the google_token and expiry_time
                    email_account.google_token = serializer.validated_data['google_token']
                    email_account.expiry_time = now() + timedelta(days=1)
                    email_account.save()
